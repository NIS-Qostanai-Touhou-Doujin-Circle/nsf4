...
// No need for useEffect here for static configuration
...
export default function WebRTCTestPage() {
    ...
    const connectSignaling = useCallback(
        (currentRoomId: string) => {
            const url = `wss://${config.BackendUrl}/signaling`;
            ...
            ws.onmessage = async (event) => {
                try {
                    ...
                    if (!peerConnectionRef.current) {
                        logMessage('local', 'PeerConnection not initialized. Ignoring message.');
                    ...
                    switch (message.type) {
                        ...
                        case 'candidate':
                            ...
                            if (message.candidate) {
                                logMessage('local', `Adding ICE candidate from ${message.from}`);
            ...
            setTimeout(async () => {
                if (
                    ...
                    try {
                        ...
                        logMessage('local', 'Error creating or sending offer: ' + error.message);
                    }
                } else {
                    logMessage(
                        'local',
